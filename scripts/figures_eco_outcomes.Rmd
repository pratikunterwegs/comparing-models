...
trait_data[, type := factor(
  type, levels = c("random", "optimal", "unevolved", "evolved")
)]
```
...
```{r}
summary_intake_move = trait_data[landscape == "patchy" & 
  dispersal == "global"][, unlist(
    lapply(.SD, function(x) {
      list(mean = mean(x), sd = sd(x))
    }
    ), recursive = FALSE
  ), by = c("type"), .SDcols = c("intake", "moved")]
...
data_sir[, type := factor(
  type, 
  levels = c("random", "optimal", "unevolved", "evolved")
)]
```
...
names(networks) = c("evolved", "optimal", "random", "unevolved")
```
...
# wrap figures
p_nets = wrap_plots(
  p_ran_net, p_opt_net, p_unevo_net, p_evo_net,
  nrow = 2,
  guides = "collect"
) &
  theme(
